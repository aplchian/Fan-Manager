...
const ListEvents = React.createClass({
  ...
  render() {
      ...
      return (
        <Link key={i} to={`/manage/events/${item._id}/show`}>
          <Panel className="event-panel" key={i}>
            ...
            <div className="panel-event-name dib relative left-2 bold">{item.name}</div>
    ...
    const resultsList = this.state.order === 'asc'
      ? map(renderEvents, sort(asc, this.state.results))
      : map(renderEvents, sort(desc, this.state.results))
      ...
      <div>
        <PageWrapper user={this.props.user} logout={this.props.logOut} title="Events">
          ...
          <Row className="show-grid">
            ...
            <Col xs={12} md={10} className="search-results pb6">
